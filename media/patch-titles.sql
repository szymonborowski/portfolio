-- =============================================================
-- Patch: update titles (remove feature-NN suffix) + update
-- feature-25 and feature-26 content (title + content change)
-- =============================================================

SET NAMES utf8mb4;
SET CHARACTER SET utf8mb4;

-- Title-only updates (feature-19 to feature-24)
UPDATE post_translations SET title = 'Komentarze, polubienia i testy', updated_at = NOW()
  WHERE locale = 'pl' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-19-comments-likes');
UPDATE post_translations SET title = 'Comments, likes and tests', updated_at = NOW()
  WHERE locale = 'en' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-19-comments-likes');
UPDATE post_translations SET title = 'Monitoring produkcyjny na Kubernetes', updated_at = NOW()
  WHERE locale = 'pl' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-20-production-monitoring');
UPDATE post_translations SET title = 'Production monitoring on Kubernetes', updated_at = NOW()
  WHERE locale = 'en' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-20-production-monitoring');
UPDATE post_translations SET title = 'Media management, lightbox i picker w edytorze', updated_at = NOW()
  WHERE locale = 'pl' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-21-media-lightbox');
UPDATE post_translations SET title = 'Media management, lightbox and editor picker', updated_at = NOW()
  WHERE locale = 'en' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-21-media-lightbox');
UPDATE post_translations SET title = 'SEO meta tagi i tłumaczenia stron', updated_at = NOW()
  WHERE locale = 'pl' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-22-seo-i18n');
UPDATE post_translations SET title = 'SEO meta tags and page translations', updated_at = NOW()
  WHERE locale = 'en' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-22-seo-i18n');
UPDATE post_translations SET title = 'Formularz kontaktowy z powiadomieniami email', updated_at = NOW()
  WHERE locale = 'pl' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-23-contact-form');
UPDATE post_translations SET title = 'Contact form with email notifications', updated_at = NOW()
  WHERE locale = 'en' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-23-contact-form');
UPDATE post_translations SET title = 'Zmiękczenie jasnego motywu', updated_at = NOW()
  WHERE locale = 'pl' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-24-light-theme');
UPDATE post_translations SET title = 'Softening the light theme', updated_at = NOW()
  WHERE locale = 'en' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-24-light-theme');

-- Full content update for feature-25 and feature-26
UPDATE post_translations SET
  title   = 'Aina Agent — od formularza kontaktowego do asystenta AI',
  excerpt = 'Portfolio miało przycisk do chatu od jakiegoś czasu. Nie robił nic. Potem feature-23 dał mu backend — formularz kontaktowy. Formularz jest użyteczny, ale też nudny. Pytanie stało się: co jeśli przycisk otwierałby rozmowę zamiast formularza?',
  content = 'Portfolio miało przycisk do chatu od jakiegoś czasu. Nie robił nic. Potem feature-23 dał mu backend — formularz kontaktowy. Formularz jest użyteczny, ale też nudny. Pytanie stało się: co jeśli przycisk otwierałby rozmowę zamiast formularza?
...
- `frontend` → `v1.22.0` (Aina Agent: AnthropicClient, VoyageClient, QdrantClient, ChatService, ChatController, widget chatu, pipeline RAG, maszyna stanów przepływu kontaktowego)',
  updated_at = NOW()
  WHERE locale = 'pl' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-25-aina-agent');
UPDATE post_translations SET
  title   = 'Aina Agent — from contact form to AI assistant',
  excerpt = 'The portfolio had a chat button for a while. It did nothing. Then feature-23 gave it a backend — a contact form. A form is useful, but a static form is also boring. The question became: what if the button opened a conversation instead of a form?',
  content = 'The portfolio had a chat button for a while. It did nothing. Then feature-23 gave it a backend — a contact form. A form is useful, but a static form is also boring. The question became: what if the button opened a conversation instead of a form?
...
That is the real argument for pao: not that it is technically clever, but that it closes a gap that quietly slows down human–agent collaboration in PHP projects.',
  updated_at = NOW()
  WHERE locale = 'en' AND post_id = (SELECT id FROM posts WHERE slug = 'feature-26-pao');
